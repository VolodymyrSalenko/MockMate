...
function OverallScoreBadge({ score }) {
  ...
  return (
    <div
      className={`w-36 h-36 rounded-full bg-gradient-to-br ${gradient} flex flex-col items-center justify-center animate-score-pop`}
      style={{
        boxShadow: `0 0 40px ${glow}, 0 0 80px ${glow.replace("0.5", "0.2")}`,
      }}
    >
      ...
      {[
        ...
        <span
          ...
          className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black transition-all ${
            star[key]
              ? "bg-gradient-to-br from-emerald-500 to-teal-400 text-white shadow-sm shadow-emerald-500/40"
...
function SpeechAnalyticsPanel({ analytics, questionIndex }) {
  ...
  const topFillers = Object.entries(analytics.fillerCounts || {})
    .sort((a, b) => b[1] - a[1])
    .slice(0, 4);
  return (
    <div className="glass border border-slate-700/40 rounded-2xl p-4 space-y-3">
      ...
      <div className="flex flex-wrap gap-2.5">
        ...
        >
          <span
            className={`font-bold text-sm ${analytics.totalFillers === 0 ? "text-emerald-400" : analytics.totalFillers <= 3 ? "text-yellow-400" : "text-red-400"}`}
          >
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {topFillers.map(([word, count]) => (
              <span
                key={word}
                className="bg-orange-500/10 border border-orange-500/25 text-orange-300 text-xs px-2.5 py-1 rounded-full"
              >
                "{word}" ×{count}
              </span>
            ))}
    ...
    >
      ...
      {item.ideal_answer && (
        <div>
          ...
          {showIdeal && (
            <div
              className="mt-2 bg-amber-500/8 border border-amber-500/20 rounded-2xl p-4 animate-fade-up"
              style={{ animationDuration: "0.2s" }}
            >
              <p className="text-xs text-amber-400 font-bold mb-1.5">
                Model Answer
              </p>
...
function OverallSpeechSummary({ qaPairs, sessionDuration }) {
  ...
  const avgWpm = Math.round(
    withAnalytics
      .filter((p) => p.analytics.wpm > 0)
      .reduce((s, p) => s + p.analytics.wpm, 0) /
      (withAnalytics.filter((p) => p.analytics.wpm > 0).length || 1),
  );
  const totalFillers = withAnalytics.reduce(
    (s, p) => s + (p.analytics.totalFillers || 0),
    0,
  );
  const totalDuration = withAnalytics.reduce(
    (s, p) => s + (p.analytics.durationSeconds || 0),
    0,
  );
  const allFillerCounts = {};
  withAnalytics.forEach((p) =>
    Object.entries(p.analytics.fillerCounts || {}).forEach(([w, c]) => {
      allFillerCounts[w] = (allFillerCounts[w] || 0) + c;
    }),
  );
  const topFiller = Object.entries(allFillerCounts).sort(
    (a, b) => b[1] - a[1],
  )[0];
  ...
  const stats = [
    ...
    {
      value: durationLabel(totalDuration),
      label: "Talk time",
      color: "text-slate-200",
    },
    {
      value: topFiller ? `"${topFiller[0]}"` : "—",
      label: "Top filler",
    ...
    ...(sessionDuration > 0
      ? [
          {
            value: durationLabel(sessionDuration),
            label: "Session time",
            color: "text-slate-200",
          },
        ]
    ...
    >
      ...
      {totalFillers === 0 && (
        <p className="text-emerald-400 text-sm font-medium">
          ✨ Zero filler words — excellent verbal discipline!
        </p>
      )}
    </div>
  );
          ...
          <Bar dataKey="words" radius={[6, 6, 0, 0]}>
            {data.map((entry, i) => (
              <Cell
                ...
                fillOpacity={0.9}
                style={{
                  filter: `drop-shadow(0 0 4px ${entry.score >= 8 ? "#10b981" : entry.score >= 5 ? "#f59e0b" : "#ef4444"})`,
                }}
              />
            ))}
          </Bar>
...
export default function Debrief({
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.save(
        `MockMate-${(role || "Interview").replace(/\s+/g, "-")}-${new Date().toISOString().slice(0, 10)}.pdf`,
      );
    } catch (e) {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const file = new File([blob], "mockmate-score.png", {
        type: "image/png",
      });
      if (navigator.canShare?.({ files: [file] })) {
        try {
          await navigator.share({
            files: [file],
            title: "My MockMate Score",
            text: `I scored ${parseFloat(debrief.overall_score).toFixed(1)}/10 on my MockMate interview!`,
          });
        } catch {
          /* user cancelled */
        }
  ...
  return (
    <div className="min-h-screen bg-slate-950 py-6 sm:py-10 px-3 sm:px-4 relative overflow-hidden">
      ...
      >
        ...
        >
          <div
            className="glass border border-emerald-500/20 rounded-3xl p-6"
          ...
          <div
            className="glass border border-amber-500/20 rounded-3xl p-6"
          ...
          <div className="space-y-4">
            {debrief.answers.map((item, i) => (
              <AnswerCard
                ...
                index={i}
                role={role}
                onRetry={(question, originalScore) =>
                  setRetryQ({ question, originalScore })
                }
              />
            ))}
          </div>
